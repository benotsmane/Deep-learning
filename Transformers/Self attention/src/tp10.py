...
class SelfAttention(nn.Module):
    def __init__(self, inputDim=50, nbClasse= 2, values_model=None, keys_model=None, query_model=None):
        ...
        if(values_model is None):
            self.values_model = nn.Linear(inputDim, inputDim, bias=True)
            
        if(keys_model is None):
            self.keys_model = nn.Linear(inputDim, inputDim, bias=True)
            
        if(query_model is None):
            self.query_model = nn.Linear(inputDim, inputDim, bias=True)
...
class SelfAttentionApprox(nn.Module):
    def __init__(self, inputDim=50, nbClasse= 2, values_model=None, keys_model=None, query_model=None, num_sample=10):
        super(SelfAttentionApprox, self).__init__()
        self.values_model = values_model
        self.keys_model = keys_model
        self.query_model = query_model
        self.sm = SMapprox(num_sample, inputDim)
...
class AttentionBlockTP10(nn.Module):
    ...
    def __init__(self, inputDim=50, nbClasse= 2, values_model=None, keys_model=None, query_model=None, mlp=None, norm=None, num_sample=0):
        
        super(AttentionBlockTP10, self).__init__()
        self.mlp = mlp
        self.selfAttention = SelfAttention(inputDim, nbClasse, values_model, keys_model, query_model) \
                            if num_sample==0 \
                            else SelfAttentionApprox(inputDim, nbClasse, values_model, keys_model, query_model, num_sample=num_sample)
            
        if(mlp is None):
            self.mlp = nn.Linear(inputDim, inputDim, bias=True)
...
class AttentionBlock(nn.Module):
    ...
    def __init__(self, inputDim=50, nbClasse= 2, values_model=None, keys_model=None, query_model=None, mlp=None, norm1=None, norm2=None):
...
class TransformerQ1Q2(nn.Module):
    ...
    def __init__(self, embeddings, word2id, inputDim=50, nbClasse= 2, L=3, context_model=None, \
                 values_model=None, keys_model=None, query_model=None, mlp=None, classifier=None, max_len=0):
        ...
        self.main = nn.Sequential(
                    *[AttentionBlockTP10(inputDim, nbClasse, \
        ...
        if(classifier is None):
            self.classifier = nn.Sequential(
                                nn.Linear(inputDim, 10, bias=True),
                                nn.ReLU(),
                                nn.Linear(10, nbClasse, bias=True)
                                )
            
        if(context_model is None):
            self.context_model = Contextualiser()
            
        self.pe = PositionalEncoding(d_model=inputDim, max_len=max_len) if max_len>0 else nn.Identity()
    ...
    def forward(self, x):
        ...
        #compute mask        
        mask = torch.where(x==self.word2id["__PAD__"], -float("Inf"), 0.).unsqueeze(2).repeat(1,1,emb_x.shape[1]).transpose(1,2)
...
class TransformerQ3(nn.Module):
    ...
    def __init__(self, embeddings, word2id, inputDim=50, nbClasse= 2, L=3, context_model=None, \
        ...
        if(embedding_cls_model is None):
            self.embedding_cls_model = nn.Linear(1, inputDim, bias=True)
...
class Transformer(nn.Module):
    ...
    def __init__(self, embeddings, word2id, inputDim=50, nbClasse= 2, L=3, context_model=None, \
        ...
        self.context_model = context_model
        self.main = nn.Sequential(
                    *[AttentionBlock(inputDim, nbClasse, \
                                     values_model=None if values_model is None else values_model[i], \
                                     keys_model=None if keys_model is None else keys_model[i],\
                                     query_model=None if query_model is None else query_model[i],\
                                     mlp=None if mlp is None else mlp[i]) \
        ...
        if(norm_layers is None):
            self.norm_layers = nn.Linear(1, inputDim, bias=True)
